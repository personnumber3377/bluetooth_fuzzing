...
// FUCK!
struct DisableGMockAtExit {
    DisableGMockAtExit() {
        ::testing::GMOCK_FLAG(catch_leaked_mocks)=false;
    }
};

static DisableGMockAtExit disable_gmock_at_exit;
...
static void FastExitNoDestructors() {
  _exit(0);
}

struct InstallFastExit {
  InstallFastExit() {
    atexit(FastExitNoDestructors);
  }
};

static InstallFastExit install_fast_exit;
*/
...
static tGATT_CBACK s_app_cback = {
    .p_conn_cb = [](tGATT_IF, const RawAddress& bda, uint16_t conn_id,
                    bool connected, tGATT_DISCONN_REASON, tBT_TRANSPORT) {
    ...
    .p_req_cb = [](uint16_t conn_id, uint32_t trans_id, tGATTS_REQ_TYPE type,
                   tGATTS_DATA* data) {
      RespondToServerRequest(conn_id, trans_id, type, data);
    },
...
/*
static tGATT_APPL_INFO s_gatt_appl_info = {
...
static void DummyNvSave(
    bool,
    tGATTS_HNDL_RANGE*)
{
}

static bool DummySrvChg(
    tGATTS_SRV_CHG_CMD,
    tGATTS_SRV_CHG_REQ*,
    tGATTS_SRV_CHG_RSP*)
{
...
static tGATT_APPL_INFO s_gatt_appl_info = {
    .p_nv_save_callback = DummyNvSave,
    .p_srv_chg_callback = DummySrvChg,
};
...
// EATT shit...
static std::vector<uint8_t> AttSignedWriteCmd(FuzzedDataProvider& fdp) {
  std::vector<uint8_t> p = {GATT_SIGN_CMD_WRITE};
  ...
  // Fake 12-byte signature field.
  auto sig = fdp.ConsumeBytes<uint8_t>(12);
  sig.resize(12);
  p.insert(p.end(), sig.begin(), sig.end());
...
class FakeBtStack {
 ...
 public:
  // NiceMock<bluetooth::testing::stack::l2cap::Mock> mock_l2cap_interface;
  // NiceMock<bluetooth::testing::stack::l2cap::Mock>* mock_l2cap_interface;
  NiceMock<bluetooth::testing::stack::l2cap::Mock>* mock_l2cap_interface = nullptr;
  /*
  FakeBtStack() {
    mock_l2cap_interface =
        new NiceMock<bluetooth::testing::stack::l2cap::Mock>();
  */

  FakeBtStack() {
    mock_l2cap_interface =
        new NiceMock<
            bluetooth::testing::stack::l2cap::Mock>();
    /*
    testing::Mock::AllowLeak(
        mock_l2cap_interface);
    */
    // Do NOT call AllowLeak.
    // Do NOT delete this pointer.
    // Do NOT reset interface at exit.
    /*
    ...
    */
    ...
    ON_CALL(*mock_l2cap_interface, L2CA_RegisterWithSecurity)
        .WillByDefault([](uint16_t psm, const tL2CAP_APPL_INFO& p_cb_info,
    ...
    ON_CALL(*mock_l2cap_interface, L2CA_RegisterLECoc)
        .WillByDefault([](uint16_t psm, Unused, Unused, Unused) {
          return psm;
        });
    */

    ON_CALL(*mock_l2cap_interface, L2CA_RegisterLECoc)
        .WillByDefault([](uint16_t psm,
                          const tL2CAP_APPL_INFO& cb,
                          Unused,
                          Unused) {
          eatt_info = cb;
          return psm;
...
static void ServerInit(bool eatt_support) {
  ...
  (void)GATTS_NVRegister(&s_gatt_appl_info);
...
static void ServerInit(bool eatt_support) {
  ...
  CHECK(s_gatt_appl_info.p_nv_save_callback);
  CHECK(s_gatt_appl_info.p_srv_chg_callback);

  LOG(INFO)
      << "appif="
      << (int)s_AppIf;

  (void)GATTS_AddService(
      s_AppIf,
      db,
...
static void CleanupGatt() {
  ...
  // gatt_free(); // TODO: Maybe comment this???
  ResetHarnessState();
}
...
/*
static void DynamicConnectPeer(
    size_t peer,
    FuzzedDataProvider& fdp)
{
    ...
    uint16_t cid =
        bluetooth::stack::l2cap::get_interface()
    ...
    // drive the callbacks after the stack created
    // real internal channel state

    if(appl_info.pL2CA_CreditBasedConnectCfm_Cb)
    {
        appl_info.pL2CA_CreditBasedConnectCfm_Cb(
            kPeerAddrs[peer],
            cid,
            cfg.mtu,
    ...
    if(appl_info.pL2CA_ConfigInd_Cb)
    {
        tL2CAP_CFG_INFO legacy_cfg{};

        legacy_cfg.mtu_present=true;
        legacy_cfg.mtu=cfg.mtu;

        appl_info.pL2CA_ConfigInd_Cb(
            cid,
            &legacy_cfg);
    }
...
static void DynamicConnectPeer(size_t peer, FuzzedDataProvider& fdp) {
  ...
  if (eatt_info.pL2CA_CreditBasedConnectInd_Cb) {
    eatt_info.pL2CA_CreditBasedConnectInd_Cb(
        kPeerAddrs[peer],
        cids,
        BT_PSM_EATT,
        247,
  ...
  if (eatt_info.pL2CA_CreditBasedConnectCfm_Cb) {
    eatt_info.pL2CA_CreditBasedConnectCfm_Cb(
        kPeerAddrs[peer],
        cid,
        247,
...
static void DynamicDisconnectPeer(size_t peer, bool ack_needed) {
  if (peer >= kMaxPeers || s_DynCid[peer] == 0 || !eatt_info.pL2CA_DisconnectInd_Cb) return;
  eatt_info.pL2CA_DisconnectInd_Cb(s_DynCid[peer], ack_needed);
  s_DynCid[peer] = 0;
}

static void FeedDynamicPeerPacket(size_t peer, const std::vector<uint8_t>& bytes) {
  if (peer >= kMaxPeers || s_DynCid[peer] == 0 || !eatt_info.pL2CA_DataInd_Cb) return;
  BT_HDR* hdr = MakeHdr(bytes);
  eatt_info.pL2CA_DataInd_Cb(s_DynCid[peer], hdr);
}
...
static void DynamicError(size_t peer, FuzzedDataProvider& fdp) {
  if (peer >= kMaxPeers || s_DynCid[peer] == 0 || !eatt_info.pL2CA_Error_Cb) return;
  eatt_info.pL2CA_Error_Cb(s_DynCid[peer], fdp.ConsumeIntegral<uint16_t>());
}
...
static void DoClientOperation(FuzzedDataProvider& fdp, size_t peer) {
  ...
  switch (fdp.ConsumeIntegralInRange<uint8_t>(0, 16)) {
    case 0: {
    ...
    */
    ...
    case 14:
    {
        ...
        DynamicCongestion(peer,true);
        DynamicCongestion(peer,false);
  ...
  case 15: {
      ...
      value.handle = PickHandle(fdp);
      value.auth_req = GATT_AUTH_REQ_SIGNED_NO_MITM;
      value.offset = 0;
  ...
  case 16: {
    ...
    FeedDynamicPeerPacket(peer, AttExchangeMtu(fdp, false));
    FeedDynamicPeerPacket(peer, AttReadReq(fdp, false));
    FeedDynamicPeerPacket(peer, AttWriteReq(fdp, GATT_REQ_WRITE));
    FeedDynamicPeerPacket(peer, AttPrepareWriteReq(fdp));
    FeedDynamicPeerPacket(peer, AttExecuteWriteReq(fdp));
...
static void FuzzAsServer(FuzzedDataProvider& fdp) {
  ...
  while (fdp.remaining_bytes() > 0) {
    ...
    uint8_t action = fdp.ConsumeIntegralInRange<uint8_t>(0, 18);
...
static void FuzzAsClient(FuzzedDataProvider& fdp) {
  ...
  while (fdp.remaining_bytes() > 0) {
    size_t peer = PickPeer(fdp, num_peers);
    switch (fdp.ConsumeIntegralInRange<uint8_t>(0, 22)) {
      case 0:
        DoClientOperation(fdp, peer);
        break;
      case 1:
